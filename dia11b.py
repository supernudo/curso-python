...
class MyApp(Frame):
    ...
    def search_in_url_after(self):
        self.after(1, self.search_in_urls)

    def search_in_url_async(self):
        Thread(target=lambda: asyncio.run(
        search_in_urls(self.url.get().split(), [self.pattern.get()]))).start()
        self.text.insert(1.0, )
